...
const inmueble_1 = require("./modelo/inmueble");
const cliente_1 = require("./modelo/cliente");
...
app.route("/inmueble").post((req, res) => {
    ...
    inmueble_1.inmueble.create(nuevo).then(() => {
...
app.route("/cliente").post((req, res) => {
    ...
    cliente_1.cliente.create(newCliente).then(() => {
...
app.route("/inmueble/:id").get((req, res) => {
    ...
    inmueble_1.inmueble.findOne({ id: id }).then((data) => {
        if (data) {
...
app.route("/cliente/:id").get((req, res) => {
    const id = req.params.id;
    cliente_1.cliente.findOne({ id: id }).then((data) => {
        if (data) {
            return res.json(data);
        }
        else {
...
app.route("/inmueble/:id").patch((req, res) => {
    ...
    inmueble_1.inmueble.findOneAndUpdate({ id: id }, { canon: nuevoCanon }).then((data) => {
...
app.route("/cliente/:id").patch((req, res) => {
    ...
    cliente_1.cliente.findOneAndUpdate({ id: id }, { celular: nuevoCel, correo: nuevoCorreo }).then((data) => {
...
app.route("/inmueble/:id").delete((req, res) => {
    const id = req.params.id;
    inmueble_1.inmueble.findOneAndDelete({ id: id }).then((data) => {
...
app.route("/cliente/:id").delete((req, res) => {
    const id = req.params.id;
    cliente_1.cliente.findOneAndDelete({ id: id }).then((data) => {
        return res.json("Se borro registro");
    }).catch(() => {
        return res.json("error");
    });
...
app.route("/cliente/:clienteId/inmueble/:inmuebleId").post((req, res) => {
    ...
    cliente_1.cliente.findOne({ id: clienteId }).then((data) => {
        if (data) {
            cliente_1.cliente.find({ "relacion.id": inmuebleId }).then((respuesta) => {
                console.log(respuesta);
                if (respuesta.length > 0) {
                    return res.json("inmueble no disponible");
                }
                else {
                    inmueble_1.inmueble.findOne({ id: inmuebleId }).then((resp) => {
                        if (resp) {
                            const relacion = {
                                id: inmuebleId,
                                estado: resp.estado
                            ...
                            cliente_1.cliente.update({ id: clienteId }, data).then((resultado) => {
